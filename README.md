...
- Покрытие тестами недостаточное для "боевого" API, на серьезном проекте я бы обязательно проверял всякие edge-case'ы в автоматическом режиме. Удачно ли заменяется фото при обновлении notebook'а? А если хотим не заменить, а удалить его? А файл точно удалился? И т.д. и т.п. В данном случае такие проверки делал "руками".
